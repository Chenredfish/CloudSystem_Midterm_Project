...
每次修改了 Python 程式（`app.py`、CLI 腳本、`ledger/`、`scripts/`）或 Dockerfile，都必須重新 build image，舊容器不會自動更新：
...
```bash
docker exec node1 python scripts/seed.py
```

只查看目前狀態（不修改資料）：

```bash
docker exec node1 python scripts/seed.py --status
```
...
```bash
docker exec node1 python scripts/tamper.py 2
```
...
輸出會逐筆列出交易，並標示每個新區塊的建立時機。完成後確認狀態：
...
向助教說明：每個區塊是一個 `.txt` 純文字檔，`Previous block` 就是前一塊的 SHA256，這就是「鏈」的連接方式。
...
```bash
docker exec node1 python app_transaction.py alice bob 500
```

輸出會顯示「（新區塊已建立）Block #21」。

繼續執行 5 筆：

```bash
docker exec node1 python app_transaction.py bob carol 400
docker exec node1 python app_transaction.py carol dave 300
docker exec node1 python app_transaction.py dave alice 200
docker exec node1 python app_transaction.py alice carol 100
docker exec node1 python app_transaction.py bob dave 50
```
...
帳戶是透過交易紀錄自動建立的（不需要事先註冊）。確認付款方曾經收過錢（有餘額）才能付款。系統初始只有 `angel` 有錢（創世區塊給了 999,999 元）。建議先執行 `seed.py` 建立測試帳戶。

**Q：CLI 指令報錯 `can't open file '/app/scripts/seed.py': No such file or directory`**
